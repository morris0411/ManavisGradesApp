...
class Exams(db.Model):
    ...
    exam_code = db.Column(
        db.Integer, db.ForeignKey("exam_master.exam_code"), nullable=False
    )
...
class SubjectScores(db.Model):
    ...
    result_id = db.Column(
        db.Integer, db.ForeignKey("exam_results.result_id"), nullable=False
    )
    subject_code = db.Column(
        db.Integer, db.ForeignKey("subject_master.subject_code"), nullable=False
    )
    score = db.Column(db.Integer, nullable=False)
    deviation_value = db.Column(db.Numeric(5, 2), nullable=False)

    exam_result = db.relationship(
        "ExamResults", backref=db.backref("subject_scores", lazy=True)
    )
    subject = db.relationship(
        "SubjectMaster", backref=db.backref("subject_scores", lazy=True)
    )
...
class Faculties(db.Model):
    ...
    university_id = db.Column(
        db.Integer, db.ForeignKey("universities.university_id"), nullable=False
    )
    faculty_name = db.Column(db.String, nullable=False)

    university = db.relationship(
        "Universities", backref=db.backref("faculties", lazy=True)
    )
...
class Departments(db.Model):
    ...
    faculty_id = db.Column(
        db.Integer, db.ForeignKey("faculties.faculty_id"), nullable=False
    )
...
class ExamJudgements(db.Model):
    ...
    exam_result = db.relationship(
        "ExamResults", backref=db.backref("exam_judgements", lazy=True)
    )
    department = db.relationship(
        "Departments", backref=db.backref("exam_judgements", lazy=True)
    )
