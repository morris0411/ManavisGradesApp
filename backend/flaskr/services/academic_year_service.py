...
def get_last_update_year():
    ...
    try:
        ...
        last_update = AcademicYearUpdate.query.order_by(
            AcademicYearUpdate.academic_year.desc()
        ).first()
        if last_update:
...
def can_update_academic_year():
    ...
    if current_date.month < 4:
        return (
            False,
            f"年度更新は4月1日以降に実行可能です（現在: {current_date.month}月）",
        )
    ...
    return (
        False,
        f"{current_year}年度の更新は既に実行済みです（最終更新: {last_update_year}年度）",
    )
...
def execute_academic_year_update():
    ...
    update_record = AcademicYearUpdate.query.filter_by(
        academic_year=current_year
    ).first()
    if update_record:
        update_record.updated_at = now
    else:
        update_record = AcademicYearUpdate(academic_year=current_year, updated_at=now)
