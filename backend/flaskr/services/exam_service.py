...
def list_exam_names(year=None, exam_type=None):
    q = db.session.query(ExamMaster.exam_name).join(
        Exams, Exams.exam_code == ExamMaster.exam_code
    )
    if year:
        q = q.filter(Exams.exam_year == year)
    if exam_type:
...
def search_exams(year=None, exam_type=None, exam_name=None):
    query = db.session.query(Exams, ExamMaster).join(
        ExamMaster, ExamMaster.exam_code == Exams.exam_code
    )
    ...
    for ex, em in exams:
        num_students = (
            db.session.query(ExamResults)
            .filter(ExamResults.exam_id == ex.exam_id)
            .count()
        )
        results.append(
            {
...
def get_exam_results(exam_id):
    rows = (
        ...
        .outerjoin(
            Departments, Departments.department_id == ExamJudgements.department_id
        )
...
def list_top_universities():
    ...
    return [
        {"university_id": u.university_id, "university_name": u.university_name}
        for u in universities
    ]


def filter_exam_results(
    exam_id,
    name=None,
    university=None,
    university_id=None,
    ...
    order_min=None,
    order_max=None,
    include_top_universities=False,
):
    # 模試タイプを取得
    ...
    if order_min and order_max:
        query = query.filter(
            ExamJudgements.preference_order.between(order_min, order_max)
        )
    ...
    if has_university_filter:
        ...
        subquery_base = (
            db.session.query(Students.student_id, ExamJudgements.preference_order)
            .join(ExamResults, ExamResults.student_id == Students.student_id)
            .join(ExamJudgements, ExamJudgements.result_id == ExamResults.result_id)
            .outerjoin(
                Departments, Departments.department_id == ExamJudgements.department_id
            ...
            .outerjoin(Faculties, Faculties.faculty_id == Departments.faculty_id)
            .outerjoin(
                Universities, Universities.university_id == Faculties.university_id
            )
            .filter(ExamResults.exam_id == exam_id)
        )
        ...
        if include_top_universities:
            ...
            subquery_base = subquery_base.filter(
                Universities.university_name.in_(TOP_UNIVERSITY_NAMES)
            )
        elif university_id:
            subquery_base = subquery_base.filter(
            ...
            subquery_base = subquery_base.filter(
                Universities.university_name.ilike(f"%{university}%")
            )
        if faculty:
            subquery_base = subquery_base.filter(
                Faculties.faculty_name.ilike(f"%{faculty}%")
            )
        if order_min and order_max:
            subquery_base = subquery_base.filter(
                ExamJudgements.preference_order.between(order_min, order_max)
        ...
        min_pref_order = func.min(
            case(
                (
                    ExamJudgements.preference_order.isnot(None),
                    ExamJudgements.preference_order,
                ),
                else_=9999,
            )
        ...
        # まず最小志望順位を取得するサブクエリ
        min_order_subquery = (
            subquery_base.with_entities(Students.student_id, min_pref_order)
        ...
        # メインクエリと同じフィルタ条件を適用
        if name:
            judgment_subquery_base = judgment_subquery_base.filter(
        ...
        judgment_subquery = (
            judgment_subquery_base.join(
                min_order_subquery,
                Students.student_id == min_order_subquery.c.student_id,
            )
            .filter(
                ExamJudgements.preference_order == min_order_subquery.c.min_pref_order
            )
        ...
        rows = (
            ...
            .outerjoin(
                judgment_subquery, Students.student_id == judgment_subquery.c.student_id
            )
            .order_by(
        ...
        rows = query.order_by(
            Students.student_id, ExamJudgements.preference_order
        ).all()
